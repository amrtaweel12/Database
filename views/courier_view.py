...
def find_available_courier(cursor, r_id):
    cursor.execute("""
        ...
        FROM Courier
        WHERE r_id = %s
        ORDER BY taskCount ASC, rating DESC
        LIMIT 1
    """, (r_id,))
...
def delivery_history_page():
    ...
    try:
        ...
        # =====================================================
        # QUERY 1: Complex Multi-Join Query (5 JOINs: 3 INNER + 2 LEFT OUTER)
        ...
        cursor.execute("""
            ...
            LEFT OUTER JOIN Menu m ON t.m_id = m.m_id
            LEFT OUTER JOIN Food f ON m.f_id = f.f_id
